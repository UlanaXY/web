...
describe('useSet', () => {
	...
	it('should render', async () => {
		const {result} = await renderHookServer(() => useSet());
	});
	...
	it('should accept initial values', async () => {
		const {result} = await renderHookServer(() => useSet([1, 2, 3]));
		const value = expectResultValue(result);
	...
	it('`add` should invoke original method and rerender component', async () => {
		...
		let i = 0;
		const {result} = await renderHookServer(() => [++i, useSet()] as const);
		const value = expectResultValue(result);
	...
	it('`delete` should invoke original method and rerender component', async () => {
		...
		let i = 0;
		const {result} = await renderHookServer(() => [++i, useSet([1])] as const);
		const value = expectResultValue(result);
