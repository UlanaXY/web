...
describe('useMap', () => {
	...
	it('should render', async () => {
		const {result} = await renderHookServer(() => useMap());
	});
	...
	it('`set` should invoke original method and not rerender component', async () => {
		...
		let i = 0;
		const {result} = await renderHookServer(() => [++i, useMap()] as const);
		const value = expectResultValue(result);
	...
	it('`delete` should invoke original method and not rerender component', async () => {
		...
		let i = 0;
		const {result} = await renderHookServer(() => [++i, useMap([['foo', 1]])] as const);
		const value = expectResultValue(result);
