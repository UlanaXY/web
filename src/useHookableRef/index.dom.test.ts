...
describe('useHookableRef', () => {
	...
	it('should return ref object with initial value', async () => {
		...
		expect(result.error).toBeUndefined();
		expect(result.value!).toEqual({current: 123});
	});
	...
	it('should persist same reference between re-renders', async () => {
		...
		await rerender();
		expect(result.value!).toBe(firstResult);
		await rerender();
		expect(result.value!).toBe(firstResult);
	});
	...
	it('should call getter and setter hook', async () => {
		...
		expect(result.value!.current).toBe(123);
		expect(getter).toHaveBeenCalledTimes(1);

		result.value!.current = 321;
		expect(result.value!.current).toBe(321);
